...
import type { TransportOfferRow } from './transport-offers-browse.component';

interface DialogData {
  offer: TransportOfferRow;
}

@Component({
  selector: 'app-transport-offer-reserve-dialog',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatDialogModule,
  ...
  template: `
    <h2 mat-dialog-title>{{ 'transport.reserve.dialogTitle' | translate }}</h2>
    <mat-dialog-content class="reserve-content">
      <div class="offer-hint">
        <div class="offer-title">{{ data.offer.title }}</div>
        <div class="offer-sub">
          {{ data.offer.transportAgencyName || '—' }} · {{ data.offer.vehiclePlate || '—' }}
        </div>
      </div>
...
export class TransportOfferReserveDialogComponent implements OnInit {
  readonly data = inject<DialogData>(MAT_DIALOG_DATA);
  private readonly dialogRef = inject(MatDialogRef<TransportOfferReserveDialogComponent, boolean>);
  ...
  private applyUnitsBoundsFromOffer(): void {
    ...
    if (minU != null) def = minU;
    else if (maxU != null) def = 1;
  ...
  submit(): void {
    ...
    this.http.post(`${this.api.transportOffers.listActive}/${this.data.offer.id}/reservations`, body).subscribe({
      next: () => {
        this.saving = false;
