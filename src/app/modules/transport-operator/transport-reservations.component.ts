...
@Component({
  selector: 'app-transport-reservations',
  standalone: true,
  imports: [MatCardModule, MatTableModule, MatButtonModule, MatIconModule, PageHeaderComponent, TranslatePipe],
  template: `
    <app-page-header
      [title]="'transport.reservationsTitle' | translate"
      [subtitle]="'transport.reservationsSubtitle' | translate"
    ></app-page-header>
            ...
            <tr class="mat-row no-data-row" *matNoDataRow>
              <td class="mat-cell" [attr.colspan]="displayedColumns.length">
                @if (loading) {
                  ...
                  <p class="muted">{{ 'transport.res.empty' | translate }}</p>
                }
              </td>
            </tr>
          </table>
  ...
  `,
  styleUrl: '../hotel-operator/hotel-reservations.component.scss',
})
export class TransportReservationsComponent implements OnInit {
  ...
  load(): void {
    this.loading = true;
    this.http.get<ReservationRow[]>(this.api.transportOperator.reservations).subscribe({
      next: (rows) => {
  ...
  setStatus(row: ReservationRow, status: ReservationStatus): void {
    ...
    this.http.post<ReservationRow>(this.api.transportOperator.reservationStatus(row.id, status), null).subscribe({
      next: (updated) => {
        row.status = (updated?.status as ReservationStatus) ?? status;
        this.savingId = null;
